...
<template>
  <div v-if="SyncRouter" class="pagination-bar">
    <template v-if="!isWebview">
      <router-link v-if="prevQuery" class="prev" title="이전" :to="{ query: prevQuery }">
      ...
      <router-link v-if="nextQuery" class="next" title="다음" :to="{ query: nextQuery }">
        <i class="ii ii-chevron-right ii-weight-600"></i>
      </router-link>
    </template>
    <!-- MARK: 웹뷰인 경우 router replace 를 해서 뒤로 가면 마켓으로 돌아가게 한다 -->
    <template v-else>
      <router-link v-if="prevQuery" class="prev" title="이전" :to="{ query: prevQuery }" replace>
        <i class="ii ii-chevron-left ii-weight-600"></i>
      </router-link>
      <router-link
        v-for="page in pages"
      ...
      <router-link v-if="nextQuery" class="next" title="다음" :to="{ query: nextQuery }" replace>
        <i class="ii ii-chevron-right ii-weight-600"></i>
      </router-link>
    </template>
  </div>
...
import { isWebview } from '../../../.storybook/utils/webview'
export default {
  props: {
    SyncRouter: {
      type: Boolean,
  ...
  methods: {
    setPages: function () {
      ...
      for (var i = 0; i < this.PagesCount; i++) {
        ...
        n = i + 1 + this.PagesCount * Math.floor(this.Start / (this.Limit * this.PagesCount))
      ...
      queryNext.start = this.pages[this.pages.length - 1].query.start + this.Limit
    ...
    reset: function () {
      ...
      this.nextPagesExist =
        this.AllItemsCount > (this.basePageNumber + this.PagesCount) * this.Limit
    ...
    toPrevPages: function () {
      ...
      this.$emit('OnPaginationChanged', { start: this.start, limit: this.limit })
    },
